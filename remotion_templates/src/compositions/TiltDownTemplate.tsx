/**
 * 文件用途：实现 TiltDownTemplate（镜头下移转场，天空→地面）。
 * 核心流程：两个场景上下排列 → 镜头带变加速度下移 → 旧场景上移出画、新场景从下方进入。
 * 输入输出：输入为 TiltDownTemplateRequest，输出为转场视频画面 JSX。
...
import type {TiltDownTemplateRequest} from "../types";
...
export const TiltDownTemplate = (props: TiltDownTemplateRequest): ReactElement => {
  const frame = useCurrentFrame();

  // 第二张放大 1.125x 居中，图片边界超出视频边界
  const SCALE = 1.125;
  // Phase 1: 快速上移到第二张上边界和视频上边界对齐（移动整高 H 保证首帧不可见）
  const phase1Dist = H;
  // Phase 2: 在放大图内从上到下慢速平移
  ...
  return (
    <AbsoluteFill style={{width: W, height: H, overflow: "hidden", filter: "grayscale(100%)"}}>
      <AbsoluteFill style={{transform: `translateY(${ty}px)`}}>
        ...
        {/* 新场景在下方（9/8 缩放，首帧不可见，phase1 结束后上边界对齐视口上侧） */}
