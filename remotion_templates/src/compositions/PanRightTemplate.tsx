/**
 * 文件用途：实现 PanRightTemplate（镜头右移转场，"下一个"）。
 * 核心流程：两个场景左右排列 → 镜头带变加速度右移 → 旧场景左移出画、新场景从右方进入。
 * 输入输出：输入为 PanRightTemplateRequest，输出为转场视频画面 JSX。
...
export const PanRightTemplate = (props: PanRightTemplateRequest): ReactElement => {
  const frame = useCurrentFrame();

  // 第二张放大 1.125x 居中，图片边界超出视频边界
  const SCALE = 1.125;
  // Phase 1: 快速右移到第二张左边界和视频左边界对齐（移动整宽 W 保证首帧不可见）
  const phase1Dist = W;
  // Phase 2: 在放大图内从左边到右边慢速平移
  const phase2Dist = W * (SCALE - 1);      // 0.125W
  ...
  const tx = -(phase1Travel + phase2Travel);
  ...
  return (
    <AbsoluteFill style={{width: W, height: H, overflow: "hidden", filter: "grayscale(100%)"}}>
      <AbsoluteFill style={{transform: `translateX(${tx}px)`}}>
        {/* 旧场景在中间 */}
        <AbsoluteFill style={{left: 0, width: W, height: H}}>
        ...
        {/* 新场景在右方（9/8 缩放，首帧不可见，phase1 结束后左边界对齐视口左侧） */}
        <AbsoluteFill style={{left: W + W * (SCALE - 1) / 2, width: W, height: H, transform: "scale(1.125)", transformOrigin: "center center"}}>
