...
class _FakeRunner:
    ...
    def retry_module_b_segment(self, task_id: str, segment_id: str, config_path: Path) -> dict:
        ...
        self.calls.append(
            (
                "retry_module_b_segment",
                {"task_id": task_id, "segment_id": segment_id, "config_path": str(config_path)},
            )
        )
        return {"task_id": task_id, "segment_id": segment_id, "kind": "b-retry"}


def test_build_parser_should_accept_module_b_status_and_retry_commands(tmp_path: Path) -> None:
    """
    功能说明：验证CLI解析器已注册模块B状态查询与segment重试命令。
    参数说明：
...
def test_dispatch_command_should_route_to_module_b_status_and_retry_methods(tmp_path: Path) -> None:
    """
    功能说明：验证CLI分发能正确调用模块B状态查询与segment重试接口。
    参数说明：
    - tmp_path: pytest 提供的临时目录。
    返回值：无。
    异常说明：断言失败时抛 AssertionError。
    ...
    status_namespace = argparse.Namespace(command="b-task-status", task_id="task_dispatch_b_001")
    ...
    assert status_result["kind"] == "b-status"
    assert fake_runner.calls[0][0] == "get_module_b_status_summary"
    assert fake_runner.calls[0][1]["task_id"] == "task_dispatch_b_001"
    assert fake_runner.calls[0][1]["config_path"] == str(config_path)

    retry_namespace = argparse.Namespace(command="b-retry-segment", task_id="task_dispatch_b_001", segment_id="seg_0003")
    ...
    assert retry_result["kind"] == "b-retry"
    assert retry_result["segment_id"] == "seg_0003"
    assert fake_runner.calls[1][0] == "retry_module_b_segment"
    assert fake_runner.calls[1][1]["task_id"] == "task_dispatch_b_001"
    assert fake_runner.calls[1][1]["segment_id"] == "seg_0003"
